#!/bin/bash
# Clean-build all 4 ECUs in Debug with the STM32CubeIDE Docker container.
...
( cd "${STM32CUBEIDEWORKSPACE}"; docker run --rm -e STM32_IMPORT_FLAG -v .:/workspace xanderhendriks/stm32cubeide:${DOCKER_STM32CUBEIDE_TAG} /workspace/scripts/build/docker_BUILD_Clean_Debug.sh ) || exit 1
