#!/bin/bash
# Incremental-build all 4 ECUs in Debug with the STM32CubeIDE Docker container.
...
if [ -e /workspace ]; then
	...
	bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUB Debug --skip-import --no-clean -D CHASSIS_LINEAR_POTENTIOMETER "$@"
