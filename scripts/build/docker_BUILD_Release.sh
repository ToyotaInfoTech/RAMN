#!/bin/bash
# Incremental-build all 4 ECUs in Release with the STM32CubeIDE Docker container.
...
if [ -e /workspace ]; then
	...
	bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUB Release --skip-import --no-clean -D CHASSIS_LINEAR_POTENTIOMETER "$@"
