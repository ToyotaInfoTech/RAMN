...
if [ -e /workspace ]; then
	...
	headless-build.sh -data /tmp/stm-workspace -build ${PROJECT_NAME}/${PROJECT_CONF} -D TARGET_ECUA
	cp -pr "${PROJECT_WORKSPACE}/${PROJECT_CONF}/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUA.hex"
	headless-build.sh -data /tmp/stm-workspace -build ${PROJECT_NAME}/${PROJECT_CONF} -D TARGET_ECUB
	cp -pr "${PROJECT_WORKSPACE}/${PROJECT_CONF}/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUB.hex"
	headless-build.sh -data /tmp/stm-workspace -build ${PROJECT_NAME}/${PROJECT_CONF} -D TARGET_ECUC
	cp -pr "${PROJECT_WORKSPACE}/${PROJECT_CONF}/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUC.hex"
	headless-build.sh -data /tmp/stm-workspace -build ${PROJECT_NAME}/${PROJECT_CONF} -D TARGET_ECUD
...
DOCKER_STM32CUBEIDE_COMMAND="docker run --rm -v .:/workspace xanderhendriks/stm32cubeide:${DOCKER_STM32CUBEIDE_VERSION} /workspace/scripts/build/docker_BUILD_Release.sh"
