...
xQueueHandle USBD_Queue_UsbRx = NULL;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	//vTaskNotifyGiveFromISR( USBD_TaskToNotify , &should_yield );
	portBASE_TYPE should_yield = pdFALSE;

	  for (uint32_t i = 0; i < *Len; ++i) {
	    if (xQueueSendFromISR(USBD_Queue_UsbRx, &Buf[i], &should_yield) == pdTRUE )
	    {
